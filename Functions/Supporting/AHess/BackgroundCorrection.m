...
% Edited by Benjamin Keedwell (April 2024)
% August 2017
...
% Fits a second order polynomial to the static flow data.
    ...
    flow_none = and((flow_std<venc/30),(flow_std>0));
    flow_mean = mean(flow,time_dim);
    fit = zeros([sz(1:3),1,size(flow, 5)]); %BK Edit: sz(flow_dim) -> size(flow, 5) to allow for a flow_dim size of 1
    ...
    for iEnc = 1:size(flow, 5) %BK Edit: sz(flow_dim) -> size(flow, 5) to allow for a flow_dim size of 1
...
%Return expanded list of input coordinates
