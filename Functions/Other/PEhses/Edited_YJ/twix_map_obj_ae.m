classdef twix_map_obj_ae < handle
...
properties(GetAccess='public', SetAccess='public')
    lBaseResolution  % the information from protocol
    ReadoutOSFactor
end
%---------------------------------------------------------------------
...
methods
    % Constructor:
    function this = twix_map_obj_ae(arg,dataType,fname,version,rstraj)
...
%         end
    ...
    function varargout = subsref(this, S)
        ...
        if this.arg.removeOS
            %if this.NCol<this.lBaseResolution*2 % only work for
            %ReadoutOSFactor=2
            %if this.NCol<this.lBaseResolution*this.ReadoutOSFactor
                outSize(1)=this.lBaseResolution;
            %end
        end
        %---------------------------------------------------------------------
    ...
    function out = readData(this,mem,cIxToTarg,cIxToRaw,selRange,selRangeSz,outSize)
        ...
        % yang, add here for aSymmetric echo downsampling
        if this.arg.removeOS
            %if this.NCol<this.lBaseResolution*2
            %if this.NCol<this.lBaseResolution*this.ReadoutOSFactor
                %keepOS       = [1:this.lBaseResolution/2, 1+this.lBaseResolution*3/2:this.lBaseResolution*2];
                keepOS       = [1:this.lBaseResolution/2, 1+this.lBaseResolution*(this.ReadoutOSFactor-1/2):this.lBaseResolution*this.ReadoutOSFactor];
            %end
        end
        %---------------------------------------------------------------------
        ...
        for k = 1:kMax
            ...
            if blockCtr == blockSz || k == kMax || (isBrokenRead && blockCtr > 1)
                ...
                if this.arg.removeOS
                    %if this.NCol<this.lBaseResolution*2
                    %if this.NCol<this.lBaseResolution*this.ReadoutOSFactor
                        %nn=this.lBaseResolution*2-this.NCol;
                        nn=this.lBaseResolution*this.ReadoutOSFactor-this.NCol;
